...
var GL_COMMAND_VERTEX_ATTRIB_DIVISOR = 98;
var GL_COMMAND_DRAW_ARRAYS_INSTANCED = 99;
var GL_COMMAND_DRAW_ELEMENTS_INSTANCED = 100;
var gl = __gl;
...
var batchGLCommands = false;
var opengl_error = 0;
var _glBindVertexArrayOES = null;
var _glDeleteVertexArrayOES = null;
var _glDrawArraysInstancedANGLE = null;
var _glDrawElementsInstancedANGLE = null;
var _glVertexAttribDivisorANGLE = null;

function batchGLCommandsToNative() {
    if (gl._flushCommands) {
        if (isSupportTypeArray()) {
            batchGLCommands = true;
...
function disableBatchGLCommandsToNative() {
    ...
    batchGLCommands = false;
    console.log('Disable batch GL commands optimization！');
    jsb.disableBatchGLCommandsToNative();
}

function enableGLParameterCheck() {
    if (!batchGLCommands) {
        console.log('Bulk GL command optimization is not enabled!');
        return;
    }
    if (parameter_check) {
        console.log('GL command has enabled parameter checking!');
        return;
    }
    console.log('GL command enables parameter checking!');
...
function activeTextureOptParamCheck(texture) {
    activeTextureOpt(texture);
}
...
function attachShaderOptParamCheck(program, shader) {
    attachShaderOpt(program, shader);
}
...
function bindAttribLocationOptParamCheck(program, index, name) {
    bindAttribLocationOpt(program, index, name);
}
...
function bindBufferOptParamCheck(target, buffer) {
    bindBufferOpt(target, buffer);
}
...
function bindFramebufferOptParamCheck(target, framebuffer) {
    if (target !== _gl.FRAMEBUFFER) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    bindFramebufferOpt(target, framebuffer);
}
...
function bindRenderbufferOptParamCheck(target, renderbuffer) {
    bindRenderbufferOpt(target, renderbuffer);
}
...
function bindTextureOptParamCheck(target, texture) {
    if (!(target === _gl.TEXTURE_2D || target === _gl.TEXTURE_CUBE_MAP)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    bindTextureOpt(target, texture);
}
...
function blendColorOptParamCheck(red, green, blue, alpha) {
    blendColorOpt(red, green, blue, alpha);
}
...
function blendEquationOptParamCheck(mode) {
    if (!(mode === _gl.FUNC_ADD || mode === _gl.FUNC_SUBTRACT || mode === _gl.FUNC_REVERSE_SUBTRACT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    blendEquationOpt(mode);
}
...
function blendEquationSeparateOptParamCheck(modeRGB, modeAlpha) {
    if (!(modeRGB === _gl.FUNC_ADD || modeRGB === _gl.FUNC_SUBTRACT || modeRGB === _gl.FUNC_REVERSE_SUBTRACT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (!(modeAlpha === _gl.FUNC_ADD || modeAlpha === _gl.FUNC_SUBTRACT || modeAlpha === _gl.FUNC_REVERSE_SUBTRACT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    blendEquationSeparateOpt(modeRGB, modeAlpha);
}
...
function blendFuncOptParamCheck(sfactor, dfactor) {
    if (sfactor === _gl.CONSTANT_COLOR && dfactor === _gl.CONSTANT_ALPHA || sfactor === _gl.ONE_MINUS_CONSTANT_COLOR && dfactor === _gl.CONSTANT_ALPHA || sfactor === _gl.CONSTANT_COLOR && dfactor === _gl.ONE_MINUS_CONSTANT_ALPHA || sfactor == _gl.ONE_MINUS_CONSTANT_COLOR && dfactor === _gl.ONE_MINUS_CONSTANT_ALPHA || sfactor === _gl.CONSTANT_ALPHA && dfactor === _gl.CONSTANT_COLOR || sfactor == _gl.CONSTANT_ALPHA && dfactor === _gl.ONE_MINUS_CONSTANT_COLOR || sfactor === _gl.ONE_MINUS_CONSTANT_ALPHA && dfactor === _gl.CONSTANT_COLOR || sfactor === _gl.ONE_MINUS_CONSTANT_ALPHA && dfactor === _gl.ONE_MINUS_CONSTANT_COLOR) {
        opengl_error = _gl.INVALID_OPERATION;
        return;
    }
    blendFuncOpt(sfactor, dfactor);
}
...
function blendFuncSeparateOptParamCheck(srcRGB, dstRGB, srcAlpha, dstAlpha) {
    if (srcRGB === _gl.CONSTANT_COLOR && dstRGB === _gl.CONSTANT_ALPHA || srcRGB === _gl.ONE_MINUS_CONSTANT_COLOR && dstRGB === _gl.CONSTANT_ALPHA || srcRGB === _gl.CONSTANT_COLOR && dstRGB === _gl.ONE_MINUS_CONSTANT_ALPHA || srcRGB === _gl.ONE_MINUS_CONSTANT_COLOR && dstRGB === _gl.ONE_MINUS_CONSTANT_ALPHA || srcRGB === _gl.CONSTANT_ALPHA && dstRGB === _gl.CONSTANT_COLOR || srcRGB === _gl.CONSTANT_ALPHA && dstRGB === _gl.ONE_MINUS_CONSTANT_COLOR || srcRGB === _gl.ONE_MINUS_CONSTANT_ALPHA && dstRGB === _gl.CONSTANT_COLOR || srcRGB === _gl.ONE_MINUS_CONSTANT_ALPHA && dstRGB === _gl.ONE_MINUS_CONSTANT_COLOR) {
        opengl_error = _gl.INVALID_OPERATION;
        return;
    }
    blendFuncSeparateOpt(srcRGB, dstRGB, srcAlpha, dstAlpha);
}
...
function bufferDataOptParamCheck(target, data, usage) {
    bufferDataOpt(target, data, usage);
}
...
function bufferSubDataOptParamCheck(target, offset, data) {
    bufferSubDataOpt(target, offset, data);
}
...
function checkFramebufferStatusOptParamCheck(target) {
    return checkFramebufferStatusOpt(target);
}
...
function clearOptParamCheck(mask) {
    clearOpt(mask);
}
...
function clearColorOptParamCheck(red, green, blue, alpha) {
    clearColorOpt(red, green, blue, alpha);
}
...
function clearDepthOptParamCheck(depth) {
    clearDepthOpt(depth);
}
...
function clearStencilOptParamCheck(s) {
    clearStencilOpt(s);
}
...
function colorMaskOptParamCheck(red, green, blue, alpha) {
    colorMaskOpt(red, green, blue, alpha);
}
...
function compileShaderOptParamCheck(shader) {
    compileShaderOpt(shader);
}
...
function compressedTexImage2DOptParamCheck(target, level, internalformat, width, height, border, data) {
    compressedTexImage2DOpt(target, level, internalformat, width, height, border, data);
}
...
function compressedTexSubImage2DOptParamCheck(target, level, xoffset, yoffset, width, height, format, data) {
    compressedTexSubImage2DOpt(target, level, xoffset, yoffset, width, height, format, data);
}
...
function copyTexImage2DOptParamCheck(target, level, internalformat, x, y, width, height, border) {
    if (!(internalformat === _gl.ALPHA || internalformat === _gl.RGB || internalformat === _gl.RGBA || internalformat === _gl.LUMINANCE || internalformat === _gl.LUMINANCE_ALPHA)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    copyTexImage2DOpt(target, level, internalformat, x, y, width, height, border);
}
...
function copyTexSubImage2DOptParamCheck(target, level, xoffset, yoffset, x, y, width, height) {
    copyTexSubImage2DOpt(target, level, xoffset, yoffset, x, y, width, height);
}
...
function createBufferOptParamCheck() {
    return createBufferOpt();
}
...
function createFramebufferOptParamCheck() {
    return createFramebufferOpt();
}
...
function createProgramOptParamCheck() {
    return createProgramOpt();
}
...
function createRenderbufferOptParamCheck() {
    return createRenderbufferOpt();
}
...
function createShaderOptParamCheck(type) {
    return createShaderOpt(type);
}
...
function createTextureOptParamCheck() {
    return createTextureOpt();
}
...
function cullFaceOptParamCheck(mode) {
    cullFaceOpt(mode);
}
...
function deleteBufferOptParamCheck(buffer) {
    deleteBufferOpt(buffer);
}
...
function deleteFramebufferOptParamCheck(framebuffer) {
    deleteFramebufferOpt(framebuffer);
}
...
function deleteProgramOptParamCheck(program) {
    deleteProgramOpt(program);
}
...
function deleteRenderbufferOptParamCheck(renderbuffer) {
    deleteRenderbufferOpt(renderbuffer);
}
...
function deleteShaderOptParamCheck(shader) {
    deleteShaderOpt(shader);
}
...
function deleteTextureOptParamCheck(texture) {
    deleteTextureOpt(texture);
}
...
function depthFuncOptParamCheck(func) {
    depthFuncOpt(func);
}
...
function depthMaskOptParamCheck(flag) {
    depthMaskOpt(flag);
}
...
function depthRangeOptParamCheck(zNear, zFar) {
    if (zNear > zFar) {
        opengl_error = _gl.INVALID_OPERATION;
        return;
    }
    depthRangeOpt(zNear, zFar);
}
...
function detachShaderOptParamCheck(program, shader) {
    detachShaderOpt(program, shader);
}
...
function disableOptParamCheck(cap) {
    disableOpt(cap);
}
...
function disableVertexAttribArrayOptParamCheck(index) {
    disableVertexAttribArrayOpt(index);
}
...
function drawArraysOptParamCheck(mode, first, count) {
    first = first >> 0;
    count = count >> 0;
    if (first < 0) {
        opengl_error = _gl.INVALID_VALUE;
        return;
    }
    drawArraysOpt(mode, first, count);
}
...
function drawElementsOptParamCheck(mode, count, type, offset) {
    count = count >> 0;
    offset = offset >> 0;
    if (!(type === _gl.UNSIGNED_BYTE || type === _gl.UNSIGNED_SHORT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (count < 0 || offset < 0) {
    ...
    drawElementsOpt(mode, count, type, offset);
}
...
function enableOptParamCheck(cap) {
    if (!(cap === _gl.BLEND || cap === _gl.CULL_FACE || cap === _gl.DEPTH_TEST || cap === _gl.DITHER || cap === _gl.POLYGON_OFFSET_FILL || cap === _gl.SAMPLE_ALPHA_TO_COVERAGE || cap === _gl.SAMPLE_COVERAGE || cap === _gl.SCISSOR_TEST || cap === _gl.STENCIL_TEST)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    enableOpt(cap);
...
function enableVertexAttribArrayOptParamCheck(index) {
    enableVertexAttribArrayOpt(index);
}
...
function framebufferRenderbufferOptParamCheck(target, attachment, renderbuffertarget, renderbuffer) {
    if (target !== _gl.FRAMEBUFFER) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (!(attachment === _gl.COLOR_ATTACHMENT0 || attachment === _gl.DEPTH_ATTACHMENT || attachment === _gl.STENCIL_ATTACHMENT || attachment === _gl.DEPTH_STENCIL_ATTACHMENT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    framebufferRenderbufferOpt(target, attachment, renderbuffertarget, renderbuffer);
}
...
function framebufferTexture2DOptParamCheck(target, attachment, textarget, texture, level) {
    if (target !== _gl.FRAMEBUFFER) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (!(attachment === _gl.COLOR_ATTACHMENT0 || attachment === _gl.DEPTH_ATTACHMENT || attachment === _gl.STENCIL_ATTACHMENT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (level !== 0) {
    ...
    framebufferTexture2DOpt(target, attachment, textarget, texture, level);
}
...
function frontFaceOptParamCheck(mode) {
    frontFaceOpt(mode);
}
...
function generateMipmapOptParamCheck(target) {
    generateMipmapOpt(target);
}
...
function getActiveAttribOptParamCheck(program, index) {
    index = index >> 0;
    return getActiveAttribOpt(program, index);
}
...
function getActiveUniformOptParamCheck(program, index) {
    index = index >> 0;
    return getActiveUniformOpt(program, index);
}
...
function getAttachedShadersOptParamCheck(program) {
    return getAttachedShadersOpt(program);
}
...
function getAttribLocationOptParamCheck(program, name) {
    return getAttribLocationOpt(program, name);
}
...
function getBufferParameterOptParamCheck(target, pname) {
    return getBufferParameterOpt(target, pname);
}
...
function getParameterOptParamCheck(pname) {
    return getParameterOpt(pname);
}
...
function getErrorOptParamCheck() {
    if (opengl_error != 0) {
        var error = opengl_error;
        opengl_error = 0;
        return error;
    }
    return getErrorOpt();
...
function getFramebufferAttachmentParameterOptParamCheck(target, attachment, pname) {
    return getFramebufferAttachmentParameterOpt(target, attachment, pname);
}
...
function getProgramParameterOptParamCheck(program, pname) {
    return getProgramParameterOpt(program, pname);
}
...
function getProgramInfoLogOptParamCheck(program) {
    return getProgramInfoLogOpt(program);
}
...
function getRenderbufferParameterOptParamCheck(target, pname) {
    return getRenderbufferParameterOpt(target, pname);
}
...
function getShaderParameterOptParamCheck(shader, pname) {
    return getShaderParameterOpt(shader, pname);
}
...
function getShaderPrecisionFormatOptParamCheck(shadertype, precisiontype) {
    return getShaderPrecisionFormatOpt(shadertype, precisiontype);
}
...
function getShaderInfoLogOptParamCheck(shader) {
    return getShaderInfoLogOpt(shader);
}
...
function getShaderSourceOptParamCheck(shader) {
    return getShaderSourceOpt(shader);
}
...
function getTexParameterOptParamCheck(target, pname) {
    return getTexParameterOpt(target, pname);
}
...
function getUniformOptParamCheck(program, location) {
    return getUniformOpt(program, location);
}
...
function getUniformLocationOptParamCheck(program, name) {
    return getUniformLocationOpt(program, name);
}
...
function getVertexAttribOptParamCheck(index, pname) {
    return getVertexAttribOpt(index, pname);
}
...
function getVertexAttribOffsetOptParamCheck(index, pname) {
    return getVertexAttribOffsetOpt(index, pname);
}
...
function hintOptParamCheck(target, mode) {
    hintOpt(target, mode);
}
...
function isBufferOptParamCheck(buffer) {
    return isBufferOpt(buffer);
}
...
function isEnabledOptParamCheck(cap) {
    return isEnabledOpt(cap);
}
...
function isFramebufferOptParamCheck(framebuffer) {
    return isFramebufferOpt(framebuffer);
}
...
function isProgramOptParamCheck(program) {
    return isProgramOpt(program);
}
...
function isRenderbufferOptParamCheck(renderbuffer) {
    return isRenderbufferOpt(renderbuffer);
}
...
function isShaderOptParamCheck(shader) {
    return isShaderOpt(shader);
}
...
function isTextureOptParamCheck(texture) {
    return isTextureOpt(texture);
}
...
function lineWidthOptParamCheck(width) {
    lineWidthOpt(width);
}
...
function linkProgramOptParamCheck(program) {
    linkProgramOpt(program);
}
...
function pixelStoreiOptParamCheck(pname, param) {
    if (pname === undefined || pname === null) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (!(pname === _gl.PACK_ALIGNMENT || pname === _gl.UNPACK_ALIGNMENT || pname === _gl.UNPACK_FLIP_Y_WEBGL || pname === _gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL || pname === _gl.UNPACK_COLORSPACE_CONVERSION_WEBGL)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    pixelStoreiOpt(pname, param);
}
...
function polygonOffsetOptParamCheck(factor, units) {
    polygonOffsetOpt(factor, units);
}
...
function readPixelsOptParamCheck(x, y, width, height, format, type, pixels) {
    readPixelsOpt(x, y, width, height, format, type, pixels);
}
...
function renderbufferStorageOptParamCheck(target, internalFormat, width, height) {
    renderbufferStorageOpt(target, internalFormat, width, height);
}
...
function sampleCoverageOptParamCheck(value, invert) {
    sampleCoverageOpt(value, invert);
}
...
function scissorOptParamCheck(x, y, width, height) {
    scissorOpt(x, y, width, height);
}
...
function shaderSourceOptParamCheck(shader, source) {
    shaderSourceOpt(shader, source);
}
...
function stencilFuncOptParamCheck(func, ref, mask) {
    stencilFuncOpt(func, ref, mask);
}
...
function stencilFuncSeparateOptParamCheck(face, func, ref, mask) {
    stencilFuncSeparateOpt(face, func, ref, mask);
}
...
function stencilMaskOptParamCheck(mask) {
    stencilMaskOpt(mask);
}
...
function stencilMaskSeparateOptParamCheck(face, mask) {
    stencilMaskSeparateOpt(face, mask);
}
...
function stencilOpOptParamCheck(fail, zfail, zpass) {
    stencilOpOpt(fail, zfail, zpass);
}
...
function stencilOpSeparateOptParamCheck(face, fail, zfail, zpass) {
    stencilOpSeparateOpt(face, fail, zfail, zpass);
}
...
function texImage2DOptParamCheck(target, level, internalformat, width, height, border, format, type, pixels) {
    var argCount = arguments.length;
    if (argCount === 6) {
        texImage2DOpt(target, level, internalformat, width, height, border);
    } else if (argCount === 9) {
        texImage2DOpt(target, level, internalformat, width, height, border, format, type, pixels);
    } else {
        console.log('texImage2DOpt: Wrong number of arguments, expected 6 or 9 but got ' + argCount);
    }
}
...
function texParameterfOptParamCheck(target, pname, param) {
    if (!(target === _gl.TEXTURE_2D || target === _gl.TEXTURE_CUBE_MAP)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    if (!(pname === _gl.TEXTURE_MIN_FILTER || pname === _gl.TEXTURE_MAG_FILTER || pname === _gl.TEXTURE_WRAP_S || pname === _gl.TEXTURE_WRAP_T)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    texParameterfOpt(target, pname, param);
}
...
function texParameteriOptParamCheck(target, pname, param) {
    if (!(target === _gl.TEXTURE_2D || target === _gl.TEXTURE_CUBE_MAP)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    ...
    texParameteriOpt(target, pname, param);
}
...
function texSubImage2DOptParamCheck(target, level, xoffset, yoffset, width, height, format, type, pixels) {
    var argCount = arguments.length;
    if (argCount === 7) {
        texSubImage2DOpt(target, level, xoffset, yoffset, width, height, format);
    } else if (argCount === 9) {
        texSubImage2DOpt(target, level, xoffset, yoffset, width, height, format, type, pixels);
    } else {
        console.log('texSubImage2DOpt: Wrong number of arguments, expected 7 or 9 but got ' + argCount);
    }
}
...
function uniform1fOpt(location, x) {
    ...
    buffer_data[next_index + 1] = location ? location._id : -1;
    buffer_data[next_index + 2] = x;
    next_index += 3;
    ++commandCount;
}

function uniform1fOptParamCheck(location, x) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform1fOpt(location, x);
}
...
function uniform2fOptParamCheck(location, x, y) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform2fOpt(location, x, y);
}
...
function uniform3fOptParamCheck(location, x, y, z) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform3fOpt(location, x, y, z);
}
...
function uniform4fOptParamCheck(location, x, y, z, w) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform4fOpt(location, x, y, z, w);
}
...
function uniform1iOptParamCheck(location, x) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform1iOpt(location, x);
}
...
function uniform2iOptParamCheck(location, x, y) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform2iOpt(location, x, y);
}
...
function uniform3iOptParamCheck(location, x, y, z) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform3iOpt(location, x, y, z);
}
...
function uniform4iOptParamCheck(location, x, y, z, w) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform4iOpt(location, x, y, z, w);
}
...
function uniform1fvOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform1fvOpt(location, value);
}
...
function uniform2fvOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform2fvOpt(location, value);
}
...
function uniform3fvOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform3fvOpt(location, value);
}
...
function uniform4fvOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform4fvOpt(location, value);
}
...
function uniform1ivOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform1ivOpt(location, value);
}
...
function uniform2ivOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform2ivOpt(location, value);
}
...
function uniform3ivOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform3ivOpt(location, value);
}
...
function uniform4ivOptParamCheck(location, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    uniform4ivOpt(location, value);
}
...
function uniformMatrix2fvOptParamCheck(location, transpose, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    if (transpose !== false) {
    ...
    uniformMatrix2fvOpt(location, transpose, value);
}
...
function uniformMatrix3fvOptParamCheck(location, transpose, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    if (transpose !== false) {
    ...
    uniformMatrix3fvOpt(location, transpose, value);
}
...
function uniformMatrix4fvOptParamCheck(location, transpose, value) {
    if (isNullOrUndefined(location)) {
        return;
    }
    if (transpose !== false) {
    ...
    uniformMatrix4fvOpt(location, transpose, value);
}
...
function useProgramOptParamCheck(program) {
    useProgramOpt(program);
}
...
function validateProgramOptParamCheck(program) {
    validateProgramOpt(program);
}
...
function vertexAttrib1fOptParamCheck(index, x) {
    vertexAttrib1fOpt(index, x);
}
...
function vertexAttrib2fOptParamCheck(index, x, y) {
    vertexAttrib2fOpt(index, x, y);
}
...
function vertexAttrib3fOptParamCheck(index, x, y, z) {
    vertexAttrib3fOpt(index, x, y, z);
}
...
function vertexAttrib4fOptParamCheck(index, x, y, z, w) {
    vertexAttrib4fOpt(index, x, y, z, w);
}
...
function vertexAttrib1fvOptParamCheck(index, value) {
    vertexAttrib1fvOpt(index, value);
}
...
function vertexAttrib2fvOptParamCheck(index, value) {
    vertexAttrib2fvOpt(index, value);
}
...
function vertexAttrib3fvOptParamCheck(index, value) {
    vertexAttrib3fvOpt(index, value);
}
...
function vertexAttrib4fvOptParamCheck(index, value) {
    vertexAttrib4fvOpt(index, value);
}
...
function vertexAttribPointerOptParamCheck(index, size, type, normalized, stride, offset) {
    if (!(type === _gl.BYTE || type === _gl.UNSIGNED_BYTE || type === _gl.SHORT || type === _gl.UNSIGNED_SHORT || type === _gl.FLOAT)) {
        opengl_error = _gl.INVALID_ENUM;
        return;
    }
    ...
    vertexAttribPointerOpt(index, size, type, normalized, stride, offset);
}
...
function viewportOptParamCheck(x, y, width, height) {
    viewportOpt(x, y, width, height);
}

function getExtensionOpt(extension) {
    var result = _gl.getExtension(extension);
    if (result instanceof OES_vertex_array_object) {
        if (_glBindVertexArrayOES === null) {
            _glBindVertexArrayOES = result.bindVertexArrayOES;
            result.bindVertexArrayOES = bindVertexArrayOESOpt;
        }
        if (_glDeleteVertexArrayOES === null) {
            _glDeleteVertexArrayOES = result.deleteVertexArrayOES;
            result.deleteVertexArrayOES = deleteVertexArrayOESOpt;
        }
    } else if (result instanceof ANGLE_instanced_arrays) {
        if (parameter_check) {
            result.drawArraysInstancedANGLE = drawArraysInstancedANGLEOptParamCheck;
            result.drawElementsInstancedANGLE = drawElementsInstancedANGLEOptParamCheck;
            result.vertexAttribDivisorANGLE = vertexAttribDivisorANGLEOptParamCheck;
        } else {
            result.drawArraysInstancedANGLE = drawArraysInstancedANGLEOpt;
            result.drawElementsInstancedANGLE = drawElementsInstancedANGLEOpt;
            result.vertexAttribDivisorANGLE = vertexAttribDivisorANGLEOpt;
        }
    }
...
function getExtensionOptParamCheck(extension) {
    return getExtensionOpt(extension);
}

function bindVertexArrayOESOpt(arrayObject) {
    flushCommands();
    _glBindVertexArrayOES(arrayObject);
}

function deleteVertexArrayOESOpt(arrayObject) {
    flushCommands();
    _glDeleteVertexArrayOES(arrayObject);
}

function drawArraysInstancedANGLEOpt(mode, first, count, primcount) {
    if (next_index + 5 >= total_size) {
        flushCommands();
    }
    ...
    buffer_data[next_index + 2] = first;
    buffer_data[next_index + 3] = count;
    buffer_data[next_index + 4] = primcount;
    next_index += 5;
    ++commandCount;
}

function drawArraysInstancedANGLEOptParamCheck(mode, first, count, primcount) {
    first = first >> 0;
    count = count >> 0;
    if (first < 0 || count < 0) {
        opengl_error = _gl.INVALID_VALUE;
    ...
    drawArraysInstancedANGLEOpt(mode, first, count, primcount);
}

function drawElementsInstancedANGLEOpt(mode, count, type, offset, primcount) {
    if (next_index + 6 >= total_size) {
        flushCommands();
    }
    ...
    buffer_data[next_index + 5] = primcount;
    next_index += 6;
    ++commandCount;
}

function drawElementsInstancedANGLEOptParamCheck(mode, count, type, offset, primcount) {
    count = count >> 0;
    offset = offset >> 0;
    if (!(type === _gl.UNSIGNED_BYTE || type === _gl.UNSIGNED_SHORT)) {
        opengl_error = _gl.INVALID_ENUM;
    ...
    drawElementsInstancedANGLEOpt(mode, count, type, offset, primcount);
}

function vertexAttribDivisorANGLEOpt(index, divisor) {
    if (next_index + 3 >= total_size) {
        flushCommands();
    }
...
function vertexAttribDivisorANGLEOptParamCheck(index, divisor) {
    vertexAttribDivisorANGLEOpt(index, divisor);
}
...
function attachMethodOptParamCheck() {
    gl.activeTexture = activeTextureOptParamCheck;
    gl.attachShader = attachShaderOptParamCheck;
    gl.bindAttribLocation = bindAttribLocationOptParamCheck;
    gl.bindBuffer = bindBufferOptParamCheck;
    gl.bindFramebuffer = bindFramebufferOptParamCheck;
    gl.bindRenderbuffer = bindRenderbufferOptParamCheck;
    gl.bindTexture = bindTextureOptParamCheck;
    gl.blendColor = blendColorOptParamCheck;
    gl.blendEquation = blendEquationOptParamCheck;
    gl.blendEquationSeparate = blendEquationSeparateOptParamCheck;
    gl.blendFunc = blendFuncOptParamCheck;
    gl.blendFuncSeparate = blendFuncSeparateOptParamCheck;
    gl.bufferData = bufferDataOptParamCheck;
    gl.bufferSubData = bufferSubDataOptParamCheck;
    gl.checkFramebufferStatus = checkFramebufferStatusOptParamCheck;
    gl.clear = clearOptParamCheck;
    gl.clearColor = clearColorOptParamCheck;
    gl.clearDepth = clearDepthOptParamCheck;
    gl.clearStencil = clearStencilOptParamCheck;
    gl.colorMask = colorMaskOptParamCheck;
    gl.compileShader = compileShaderOptParamCheck;
    gl.compressedTexImage2D = compressedTexImage2DOptParamCheck;
    gl.compressedTexSubImage2D = compressedTexSubImage2DOptParamCheck;
    gl.copyTexImage2D = copyTexImage2DOptParamCheck;
    gl.copyTexSubImage2D = copyTexSubImage2DOptParamCheck;
    gl.createBuffer = createBufferOptParamCheck;
    gl.createFramebuffer = createFramebufferOptParamCheck;
    gl.createProgram = createProgramOptParamCheck;
    gl.createRenderbuffer = createRenderbufferOptParamCheck;
    gl.createShader = createShaderOptParamCheck;
    gl.createTexture = createTextureOptParamCheck;
    gl.cullFace = cullFaceOptParamCheck;
    gl.deleteBuffer = deleteBufferOptParamCheck;
    gl.deleteFramebuffer = deleteFramebufferOptParamCheck;
    gl.deleteProgram = deleteProgramOptParamCheck;
    gl.deleteRenderbuffer = deleteRenderbufferOptParamCheck;
    gl.deleteShader = deleteShaderOptParamCheck;
    gl.deleteTexture = deleteTextureOptParamCheck;
    gl.depthFunc = depthFuncOptParamCheck;
    gl.depthMask = depthMaskOptParamCheck;
    gl.depthRange = depthRangeOptParamCheck;
    gl.detachShader = detachShaderOptParamCheck;
    gl.disable = disableOptParamCheck;
    gl.disableVertexAttribArray = disableVertexAttribArrayOptParamCheck;
    gl.drawArrays = drawArraysOptParamCheck;
    gl.drawElements = drawElementsOptParamCheck;
    gl.enable = enableOptParamCheck;
    gl.enableVertexAttribArray = enableVertexAttribArrayOptParamCheck;
    gl.finish = finishOptParamCheck;
    gl.flush = flushOptParamCheck;
    gl.framebufferRenderbuffer = framebufferRenderbufferOptParamCheck;
    gl.framebufferTexture2D = framebufferTexture2DOptParamCheck;
    gl.frontFace = frontFaceOptParamCheck;
    gl.generateMipmap = generateMipmapOptParamCheck;
    gl.getActiveAttrib = getActiveAttribOptParamCheck;
    gl.getActiveUniform = getActiveUniformOptParamCheck;
    gl.getAttachedShaders = getAttachedShadersOptParamCheck;
    gl.getAttribLocation = getAttribLocationOptParamCheck;
    gl.getBufferParameter = getBufferParameterOptParamCheck;
    gl.getParameter = getParameterOptParamCheck;
    gl.getError = getErrorOptParamCheck;
    gl.getFramebufferAttachmentParameter = getFramebufferAttachmentParameterOptParamCheck;
    gl.getProgramParameter = getProgramParameterOptParamCheck;
    gl.getProgramInfoLog = getProgramInfoLogOptParamCheck;
    gl.getRenderbufferParameter = getRenderbufferParameterOptParamCheck;
    gl.getShaderParameter = getShaderParameterOptParamCheck;
    gl.getShaderPrecisionFormat = getShaderPrecisionFormatOptParamCheck;
    gl.getShaderInfoLog = getShaderInfoLogOptParamCheck;
    gl.getShaderSource = getShaderSourceOptParamCheck;
    gl.getTexParameter = getTexParameterOptParamCheck;
    gl.getUniform = getUniformOptParamCheck;
    gl.getUniformLocation = getUniformLocationOptParamCheck;
    gl.getVertexAttrib = getVertexAttribOptParamCheck;
    gl.getVertexAttribOffset = getVertexAttribOffsetOptParamCheck;
    gl.hint = hintOptParamCheck;
    gl.isBuffer = isBufferOptParamCheck;
    gl.isEnabled = isEnabledOptParamCheck;
    gl.isFramebuffer = isFramebufferOptParamCheck;
    gl.isProgram = isProgramOptParamCheck;
    gl.isRenderbuffer = isRenderbufferOptParamCheck;
    gl.isShader = isShaderOptParamCheck;
    gl.isTexture = isTextureOptParamCheck;
    gl.lineWidth = lineWidthOptParamCheck;
    gl.linkProgram = linkProgramOptParamCheck;
    gl.pixelStorei = pixelStoreiOptParamCheck;
    gl.polygonOffset = polygonOffsetOptParamCheck;
    gl.readPixels = readPixelsOptParamCheck;
    gl.renderbufferStorage = renderbufferStorageOptParamCheck;
    gl.sampleCoverage = sampleCoverageOptParamCheck;
    gl.scissor = scissorOptParamCheck;
    gl.shaderSource = shaderSourceOptParamCheck;
    gl.stencilFunc = stencilFuncOptParamCheck;
    gl.stencilFuncSeparate = stencilFuncSeparateOptParamCheck;
    gl.stencilMask = stencilMaskOptParamCheck;
    gl.stencilMaskSeparate = stencilMaskSeparateOptParamCheck;
    gl.stencilOp = stencilOpOptParamCheck;
    gl.stencilOpSeparate = stencilOpSeparateOptParamCheck;
    gl.texImage2D = texImage2DOptParamCheck;
    gl.texParameterf = texParameterfOptParamCheck;
    gl.texParameteri = texParameteriOptParamCheck;
    gl.texSubImage2D = texSubImage2DOptParamCheck;
    gl.uniform1f = uniform1fOptParamCheck;
    gl.uniform2f = uniform2fOptParamCheck;
    gl.uniform3f = uniform3fOptParamCheck;
    gl.uniform4f = uniform4fOptParamCheck;
    gl.uniform1i = uniform1iOptParamCheck;
    gl.uniform2i = uniform2iOptParamCheck;
    gl.uniform3i = uniform3iOptParamCheck;
    gl.uniform4i = uniform4iOptParamCheck;
    gl.uniform1fv = uniform1fvOptParamCheck;
    gl.uniform2fv = uniform2fvOptParamCheck;
    gl.uniform3fv = uniform3fvOptParamCheck;
    gl.uniform4fv = uniform4fvOptParamCheck;
    gl.uniform1iv = uniform1ivOptParamCheck;
    gl.uniform2iv = uniform2ivOptParamCheck;
    gl.uniform3iv = uniform3ivOptParamCheck;
    gl.uniform4iv = uniform4ivOptParamCheck;
    gl.uniformMatrix2fv = uniformMatrix2fvOptParamCheck;
    gl.uniformMatrix3fv = uniformMatrix3fvOptParamCheck;
    gl.uniformMatrix4fv = uniformMatrix4fvOptParamCheck;
    gl.useProgram = useProgramOptParamCheck;
    gl.validateProgram = validateProgramOptParamCheck;
    gl.vertexAttrib1f = vertexAttrib1fOptParamCheck;
    gl.vertexAttrib2f = vertexAttrib2fOptParamCheck;
    gl.vertexAttrib3f = vertexAttrib3fOptParamCheck;
    gl.vertexAttrib4f = vertexAttrib4fOptParamCheck;
    gl.vertexAttrib1fv = vertexAttrib1fvOptParamCheck;
    gl.vertexAttrib2fv = vertexAttrib2fvOptParamCheck;
    gl.vertexAttrib3fv = vertexAttrib3fvOptParamCheck;
    gl.vertexAttrib4fv = vertexAttrib4fvOptParamCheck;
    gl.vertexAttribPointer = vertexAttribPointerOptParamCheck;
    gl.viewport = viewportOptParamCheck;
    gl.getExtension = getExtensionOptParamCheck;
}

function attachMethodOptNormal() {
    ...
    gl.getExtension = getExtensionOpt;
}

function attachMethodOpt() {
    if (parameter_check) {
        attachMethodOptParamCheck();
    } else {
        attachMethodOptNormal();
    }
}
...
module.exports = {
    ...
    enableGLParameterCheck: enableGLParameterCheck,
...
var _require3 = require('./glOptMode'),
    ...
    enableGLParameterCheck = _require3.enableGLParameterCheck,
...
},{"./Blob":1,"./base64/base64.min":2,"./glOptMode":3,"./jsb-adapter":26,"./jsb_audioengine":31,"./jsb_input":32,"./jsb_opengl":33,"./jsb_prepare":35,"./xmldom/dom-parser":36}],5:[function(require,module,exports){
"use strict";
...
},{"./DOMRect":5,"./Node":23}],8:[function(require,module,exports){
"use strict";
...
function _getTouchIndex(touch) {
    var element;
    for (var index = 0; index < _touchPointList.length; index++) {
        element = _touchPointList[index];
...
function _copyObject(fromObj, toObject) {
    ...
    return function (touches) {
        ...
        if (type === "touchstart") {
            touches.forEach(function (element) {
                ...
                if (index >= 0) {
                    _copyObject(element, _touchPointList[index]);
                } else {
                    var tmp = {};
                    _copyObject(element, tmp);
                    _touchPointList.push(tmp);
                }
            });
            ...
            touches.forEach(function (element) {
                index = _getTouchIndex(element);
                if (index >= 0) {
                    _touchPointList.splice(index, 1);
                }
            });
        }
        touchEvent.touches = _touchPointList;
        touchEvent.targetTouches = Array.prototype.slice.call(_touchPointList);
...
var dirtyDraws = { "clearRect": true, "fillRect": true, "strokeRect": true };

var CanvasGradient = jsb.CanvasGradient;
delete jsb.CanvasGradient;

var gradientProto = CanvasGradient.prototype;
var origin_addColorStop = gradientProto.addColorStop;
gradientProto.addColorStop = function (offset, color) {
    var pos = Number(offset);
    if (isNaN(pos) || pos < 0 || pos > 1) {
        throw IndexSizeError;
    }
    origin_addColorStop.apply(this, [pos, color]);
};

var ctx2DProto = CanvasRenderingContext2D.prototype;

ctx2DProto.createLinearGradient = function (x0, y0, x1, y1) {
    return new CanvasGradient(x0, y0, x1, y1, -1, -1);
};

ctx2DProto.createRadialGradient = function () {
    for (var len = arguments.length, rest = Array(len), i = 0; i < len; i++) {
        rest[i] = arguments[i];
        if (isNaN(rest[i]) || !isFinite(rest[i])) {
            return;
        }
        rest[i] = Number(rest[i]);
    }
    if (rest[0] == rest[3] && rest[1] == rest[4] && rest[2] == rest[5]) {
        return;
    }
    if (rest[2] < 0 || rest[5] < 0) {
        throw IndexSizeError;
    }
    return new CanvasGradient(rest[0], rest[1], rest[3], rest[4], rest[2], rest[5]);
};

var CanvasPattern = jsb.CanvasPattern;
delete jsb.CanvasPattern;
ctx2DProto.createPattern = function (image, repeat_rule) {
    if (image === undefined || image === null) {
        console.error("Invalid image argument passed to createPattern!");
        return null;
    }

    var data = image._imageMeta;
    var width = image.width;
    var height = image.height;

    if (image instanceof HTMLCanvasElement) {
        if (image._data) {
            data = image._data.data;
        }
        if (data === undefined) {
    ...
    if (data === undefined || data === null || width === undefined || width <= 0 || height === undefined || height <= 0) {
        console.error("Invalid image argument passed to createPattern!");
        return null;
    }
    if (repeat_rule === undefined || repeat_rule == "") {
        repeat_rule = "repeat";
    }
    if (repeat_rule !== "repeat" && repeat_rule !== "repeat-x" && repeat_rule !== "repeat-y" && repeat_rule !== "no-repeat") {
        throw SyntaxError;
    }
    return new CanvasPattern(width, height, data, repeat_rule);
};

ctx2DProto.isPointInPath = function (x, y) {
    return false;
};

ctx2DProto.isPointInStroke = function (x, y) {
    return false;
};

rewriteMethodForMarkDirty(ctx2DProto, 'fill');
rewriteMethodForMarkDirty(ctx2DProto, 'stroke');
rewriteMethodForCheckNumArgs(ctx2DProto, 'moveTo');
rewriteMethodForCheckNumArgs(ctx2DProto, 'lineTo');
rewriteMethodForCheckNumArgs(ctx2DProto, 'clearRect');
rewriteMethodForCheckNumArgs(ctx2DProto, 'fillRect');
rewriteMethodForCheckNumArgs(ctx2DProto, 'rect');
rewriteMethodForCheckNumArgs(ctx2DProto, 'quadraticCurveTo');
rewriteMethodForCheckNumArgs(ctx2DProto, 'scale');
rewriteMethodForCheckNumArgs(ctx2DProto, 'translate');
rewriteMethodForCheckNumArgs(ctx2DProto, 'rotate');
rewriteMethodForCheckNumArgs(ctx2DProto, 'transform');
rewriteMethodForCheckNumArgs(ctx2DProto, 'setTransform');
rewriteMethodForCheckNumArgs(ctx2DProto, 'strokeRect');
rewriteMethodForCheckNumArgs(ctx2DProto, 'bezierCurveTo');
rewriteMethodForCheckNumArgsOfSetLineDash(ctx2DProto);
rewriteMethodForCheckNumArgsOfArc(ctx2DProto, "arc");
rewriteMethodForCheckNumArgsOfArc(ctx2DProto, "arcTo");
rewriteMethodForCheckNumArgsOfEllipse(ctx2DProto);
rewriteDrawImageForCheckAndCalcArgs(ctx2DProto);
rewriteMethodForCheckNumArgsOfDrawText(ctx2DProto, "fillText");
rewriteMethodForCheckNumArgsOfDrawText(ctx2DProto, "strokeText");
rewriteFieldForCheckNumParams(ctx2DProto, "lineDashOffset");
rewriteFieldForCheckNumParams(ctx2DProto, "miterLimit", true);
rewriteFieldForCheckNumParams(ctx2DProto, "lineWidth", true);
rewriteFieldForCheckNumParams(ctx2DProto, "shadowOffsetX");
rewriteFieldForCheckNumParams(ctx2DProto, "shadowOffsetY");
rewriteFieldForCheckNumParams(ctx2DProto, "shadowBlur", true);
rewriteFieldForCheckNumParams(ctx2DProto, "globalAlpha");

function rewriteMethodForCheckNumArgs(prototype, key) {
    ...
    prototype[key] = function () {
        ...
        if (dirtyDraws[key]) {
            this._innerCanvas._dataInner = null;
        }
    };
}

function rewriteMethodForMarkDirty(prototype, key) {
    var method = prototype[key];
    prototype[key] = function () {
        method.apply(this);
        this._innerCanvas._dataInner = null;
...
function rewriteMethodForCheckNumArgsOfSetLineDash(prototype) {
    var method = prototype["setLineDash"];
    prototype["setLineDash"] = function () {
        if (arguments.length < 1) {
            return;
        }
        var arr = [];
        var tmp = arguments[0];
        for (var len = tmp.length, i = 0; i < len; i++) {
            if (isNaN(tmp[i]) || !isFinite(tmp[i])) {
                return;
            }
            var value = Number(tmp[i]);
            if (value < 0) {
                return;
            }
            arr[i] = value;
...
function rewriteMethodForCheckNumArgsOfArc(prototype, key) {
    var method = prototype[key];
    var isArcTo = key === "arcTo";
    prototype[key] = function () {
        for (var len = arguments.length, rest = Array(len), i = 0; i < len; i++) {
            rest[i] = arguments[i];
            if (isNaN(rest[i]) || !isFinite(rest[i])) {
        ...
        if (isArcTo && rest[4] < 0 || !isArcTo && rest[2] < 0) {
            throw IndexSizeError;
        }
        method.apply(this, rest);
    };
...
function rewriteMethodForCheckNumArgsOfEllipse(prototype) {
    var method = prototype["ellipse"];
...
function rewriteDrawImageForCheckAndCalcArgs(prototype) {
    var _drawImage = prototype['drawImage'];
    ...
    prototype['drawImage'] = function () {
        ...
        if (image === undefined || image === null) {
            console.error("Invalid image argument passed to drawImage!");
            return;
        }
        ...
        if (image instanceof HTMLImageElement) {
            data = image._imageMeta;
            originalWidth = sw = image.width;
            originalHeight = sh = image.height;
        ...
        if (data === undefined || data === null || originalWidth <= 0 || originalHeight <= 0) {
            console.error("Invalid image argument passed to drawImage!");
            return;
        }

        if (argCount === 3) {
            dx = Number(arguments[1]);
            dy = Number(arguments[2]);
            dw = sw;
            dh = sh;
        } else if (argCount === 5) {
            dx = Number(arguments[1]);
            dy = Number(arguments[2]);
            dw = Number(arguments[3]);
            dh = Number(arguments[4]);
            scale = true;
        } else if (argCount === 9) {
            sx = Number(arguments[1]);
            sy = Number(arguments[2]);
            sw = Number(arguments[3]);
            sh = Number(arguments[4]);
            dx = Number(arguments[5]);
            dy = Number(arguments[6]);
            dw = Number(arguments[7]);
            dh = Number(arguments[8]);
        } else {
            console.error(new Error("drawImage: invalid argument count!").stack);
            return;
        }

        if (sx > originalWidth || sy > originalHeight) {
            return;
        }

        if (!scale) {
            if (sx + sw > originalWidth) {
                dw = dw * (originalWidth - sx) / sw;
                sw = originalWidth - sx;
            }

            if (sy + sh > originalHeight) {
                dh = dh * (originalHeight - sy) / sh;
                sh = originalHeight - sy;
            }
        ...
        _drawImage.apply(this, [data, sx, sy, sw, sh, dx, dy, dw, dh, originalWidth, originalHeight]);
        this._innerCanvas._dataInner = null;
    };
}

function rewriteFieldForCheckNumParams(prototype, fieldName, checkNegative) {
    var filed = fieldName + "Internal";
    Object.defineProperty(prototype, fieldName, {
        set: function set(value) {
            ...
            if (checkNegative == true && num <= 0) {
                return;
            }
            this[filed] = Number(value);
        },
        get: function get() {
            return this[filed];
        }
...
function rewriteMethodForCheckNumArgsOfDrawText(prototype, methodName) {
    var method = prototype[methodName];
    prototype[methodName] = function () {
        var argCount = arguments.length;
        var rest = Array(argCount);
        rest[0] = arguments[0];
        for (var i = 1; i < argCount; i++) {
            if (isNaN(arguments[i]) || !isFinite(arguments[i])) {
                return;
            }
            rest[i] = Number(arguments[i]);
        }
...
},{"./DOMRect":5,"./HTMLElement":14}],14:[function(require,module,exports){
'use strict';
...
},{"./Element":7,"./util":29}],15:[function(require,module,exports){
'use strict';
...
var HTMLImageElement = function (_HTMLElement) {
    ...
    _createClass(HTMLImageElement, [{
        ...
        set: function set(src) {
            ...
            loadRuntime().loadImageData(src, function (info) {
                if (!info) {
...
},{"./HTMLElement":14,"./MediaError":21}],17:[function(require,module,exports){
'use strict';
...
},{"./Event":8}],21:[function(require,module,exports){
"use strict";
...
},{"./FontFaceSet":12,"./HTMLCanvasElement":13,"./HTMLElement":14,"./HTMLScriptElement":17,"./HTMLVideoElement":18,"./Image":19,"./Node":23,"./location":27}],26:[function(require,module,exports){
'use strict';
...
function inject() {
    ...
    Object.defineProperty(window.__canvas, "width", {
        set: function set() {},
        get: function get() {
            return window.innerWidth;
        }
    });
    Object.defineProperty(window.__canvas, "height", {
        set: function set() {},
        get: function get() {
    ...
    window.resize = function (width, height) {
        ...
        if (canvas._context2D) {
            canvas._context2D._width = window.innerWidth;
            canvas._context2D._height = window.innerHeight;
        }
        canvas._alignment = canvas.width % 2 === 0 ? 8 : 4;
        canvas._dataInner = null;
...
},{"./DeviceMotionEvent":6,"./Element":7,"./Event":8,"./EventTarget":9,"./FileReader":10,"./FontFace":11,"./FontFaceSet":12,"./HTMLCanvasElement":13,"./HTMLElement":14,"./HTMLImageElement":15,"./HTMLMediaElement":16,"./HTMLScriptElement":17,"./HTMLVideoElement":18,"./Image":19,"./KeyboardEvent":20,"./MouseEvent":22,"./TouchEvent":24,"./document":25,"./location":27,"./navigator":28}],31:[function(require,module,exports){
"use strict";
...
},{"./jsb-adapter/Event":8,"./jsb-adapter/EventTarget":9}],33:[function(require,module,exports){
'use strict';
...
var _glGetExtension = gl.getExtension;
...
gl.getExtension = function (extension) {
    ...
    if (supportedExtensions.indexOf(extension) > -1) {
        ...
        return _glGetExtension(extension);
    }
...
var _glDrawArrays = gl.drawArrays;
...
var _glDrawElements = gl.drawElements;
...
var _glGetActiveAttrib = gl.getActiveAttrib;

gl.getActiveAttrib = function (program, index) {
    index = index >> 0;
    return _glGetActiveAttrib(program, index);
};

var _glGetActiveUniform = gl.getActiveUniform;

gl.getActiveUniform = function (program, index) {
    index = index >> 0;
    return _glGetActiveUniform(program, index);
};

var _glGetProgramInfoLog = gl.getProgramInfoLog;

gl.getProgramInfoLog = function () {
    if (arguments.length >= 1) {
        return _glGetProgramInfoLog(arguments[0]);
    } else {
        return _glGetProgramInfoLog();
    }
};

var _glGetShaderInfoLog = gl.getShaderInfoLog;

gl.getShaderInfoLog = function () {
    if (arguments.length >= 1) {
        return _glGetShaderInfoLog(arguments[0]);
    } else {
        return _glGetShaderInfoLog();
    }
};

},{"./jsb-adapter/HTMLCanvasElement":13,"./jsb-adapter/HTMLImageElement":15,"./jsb_opengl_constants":34}],34:[function(require,module,exports){
"use strict";
