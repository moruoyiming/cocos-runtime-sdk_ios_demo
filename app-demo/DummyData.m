...
- (instancetype)initWithJSON:(NSDictionary *)json
{
    self = [super init];
    if (self) {
        self.orientation = [[json objectForKey:KEY_ORIENTATION] integerValue];
        self.screenMode = [[json objectForKey:KEY_SCREEN_MODE] integerValue];
        self.version = [NSString stringWithFormat:@"%@",[json objectForKey:KEY_VERSION]];
        
        self.appID = [json objectForKey:KEY_APP_ID];
        self.packageHash = [json objectForKey:KEY_HASH];
        self.icon = [GameEnv buildIconURL:self.appID];
        self.name = [json objectForKey:KEY_NAME];
        self.url = [GameEnv buildGamePackageRequest:self.appID version:self.version];
        
        self.tags = [json objectForKey:KEY_TAG];
        self.require = [json objectForKey:KEY_REQUIRE];
    }
    return self;
}
@end
