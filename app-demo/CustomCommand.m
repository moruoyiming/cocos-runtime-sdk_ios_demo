...
- (void)onCallCustomCommand:(id<CRGameCustomCommandHandle>)handle info:(NSDictionary *)argv {
    ...
    for (int i = 0; i < count; i++) {
        ...
        NSString *type = [argv objectForKey:key];
        NSString *index = [NSString stringWithFormat:@"%d",i];
        if ([@"boolean" isEqualToString:type]) {
            [handle pushResultWithBool:[[argv objectForKey:index] boolValue]];
        } else if ([@"string" isEqualToString:type]) {
            [handle pushResultWithString:[argv objectForKey:index]];
        } else if ([@"long" isEqualToString:type]) {
            [handle pushResultWithLong:[[argv objectForKey:index] longValue]];
        } else if ([@"double" isEqualToString:type]) {
            [handle pushResultWithDouble:[[argv objectForKey:index] doubleValue]];
        } else if ([@"Int8Array" isEqualToString:type]) {
            [handle pushResultWithInt8Arr:[argv objectForKey:index]];
        } else if ([@"Int16Array" isEqualToString:type]) {
            [handle pushResultWithInt16Arr:[argv objectForKey:index]];
        } else if ([@"Int32Array" isEqualToString:type]) {
            [handle pushResultWithInt32Arr:[argv objectForKey:index]];
        } else if ([@"Float32Array" isEqualToString:type]) {
            [handle pushResultWithFloatArr:[argv objectForKey:index]];
        } else if ([@"Float64Array" isEqualToString:type]) {
            [handle pushResultWithDoubleArr:[argv objectForKey:index]];
        } else if ([@"[boolean]" isEqualToString:type]) {
            [handle pushResultWithBoolArr:[argv objectForKey:index]];
        } else if ([@"[string]" isEqualToString:type]) {
            [handle pushResultWithStringArr:[argv objectForKey:index]];
        } else if ([@"[number]" isEqualToString:type]) {
            NSArray *numberArr = [argv objectForKey:index];
            NSUInteger length = [numberArr count];
            double arr[length];
            for (int j = 0; j < [numberArr count]; j++) {
                double value = [[numberArr objectAtIndex:j] doubleValue];
                arr[j] = value;
            }
            NSData *data = [NSData dataWithBytes:arr length:length*sizeof(double)];
            [handle pushResultWithDoubleArr:data];
        } else if ([@"null" isEqualToString:type]) {
            [handle pushResultNull];
        }
    }
